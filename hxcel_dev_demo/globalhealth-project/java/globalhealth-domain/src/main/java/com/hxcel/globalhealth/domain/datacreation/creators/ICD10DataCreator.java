 ...
 */
public class ICD10DataCreator extends AbstractDataCreator implements DataCreator {
    private final static Logger log = LoggerFactory.getLogger(ICD10DataCreator.class);
    private static int populated = 0;
    private static int omitted = 0;
    private static final String DIAGNOSIS_VERSION = "revision_10_2nd_edition";

    public void create() throws DataCreatorException {

        try {
            Integer count = journalManager.getDiagnosisCount(DiagnosisTypeCd.ICD10);
        ...
        log.info("Populated " + populated + " icd-10 records in db");
        log.info("Omitted " + omitted + " icd-10 from db. Already exists.");
    }
    ...
    private List<Diagnosis> parseXMLFile() throws IOException, DataCreatorException {
        ...
        try {
            ...
            List<Element> diagnoses = document.selectNodes("//icd10/diagnosis");
            Node nCountry = document.selectSingleNode("//icd10");
            ...
            for (Element e : diagnoses) {
                ...
                if (cells.size() == 2) {
                    ...
                    diagnosis.setDiagnosisType(DiagnosisTypeCd.ICD10);
     ...
     *
     */
    private void persist(List<Diagnosis> icd10) throws DataCreatorException {
        try {

            log.info("Persisting " + icd10.size() + " records. This might take some time the first time you populate the db.");
            // blow away existing icd entries  only way
            journalManager.saveOrUpdateAllDiagnoses(icd10);
            populated = icd10.size();
