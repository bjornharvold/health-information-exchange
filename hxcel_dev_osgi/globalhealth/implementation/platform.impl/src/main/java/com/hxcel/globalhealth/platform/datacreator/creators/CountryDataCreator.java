...
public class CountryDataCreator extends AbstractDataCreator implements DataCreator {
    ...
    public void create() throws DataCreatorException {
        ...
        try {
            Integer count = countryService.searchForCountriesCount(null);
    ...
    private List<ICountryDto> parseCSVFile() throws IOException {
        List<ICountryDto> result = new ArrayList<ICountryDto>();
        ...
        try {
            ...
            for (Element e : countries) {
                ...
                if (cells.size() == 11) {
                    ...
                    country.setStatusCode(statusCode);
                    country.setCurrencyCode(currencyCode);
                    country.setCurrencyCodeNumber(currencyCodeNumber.equals("null") ? null : Integer.parseInt(currencyCodeNumber));
                    country.setCurrencyName(currencyName);
                    country.setExchangeRate(exchangeRate.equals("null") ? null : BigDecimal.valueOf(Double.parseDouble(exchangeRate)));
                    country.setExchangeDate(new Date());
                    country.setDateFormatPattern(dateFormatPattern);
                    country.setTimeFormatPattern(timeFormatPattern);
                    country.setTimestampFormatPattern(timestampFormatPattern);
     ...
     */
    private void persist(List<ICountryDto> countries) throws DataCreatorException {
        List<ICountryDto> dbList = new ArrayList<ICountryDto>();
        try {

            for (ICountryDto country : countries) {
                CountryCd c = CountryCd.valueOf(country.getStatusCode());

                if (c != null) {
                    ICountryDto existingCountry = countryService.getCountry(c);
...
//                        }
            ...
            // ready fr save all
            if (dbList.size() > 0) {
                for (ICountryDto country : dbList) {
